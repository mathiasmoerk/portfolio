          ...
          <div className="flex flex-wrap gap-2">
            {site.social.map((s) => (
              <a
                key={s.label}
                href={s.href}
                className="inline-flex h-12 items-center rounded-full border border-paper/20 px-6 text-xs font-medium uppercase tracking-widest text-paper/80 transition-colors hover:border-accent hover:bg-accent hover:text-white"
              >
                {s.label}
              </a>
            ))}
